...
# Code from: https://github.com/ChunML/NLP/blob/master/machine_translation/train_transformer_tf2.py
...
with open("/tmp/train.en") as f1:
    raw_data_en = [lines.rstrip("\n") for lines in f1]
with open("/tmp/train.fr") as f1:
    raw_data_fr = [lines.rstrip("\n") for lines in f1]
...
class MultiHeadAttention(tf.keras.Model):
    ...
    def call(self, query, value, mask=None):
        ...
        score = tf.matmul(query, key, transpose_b=True) / tf.math.sqrt(
            tf.dtypes.cast(self.key_size, dtype=tf.float32)
        )
...
class Encoder(tf.keras.Model):
    ...
    def __init__(self, vocab_size, model_size, num_layers, h):
        ...
        self.dense_1 = [
            tf.keras.layers.Dense(MODEL_SIZE * 4, activation="relu")
...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, vocab_size, model_size, num_layers, h):
        ...
        self.embedding_dropout = tf.keras.layers.Dropout(0.1)
        self.attention_bot = [
            MultiHeadAttention(model_size, h) for _ in range(num_layers)
        ]
...
for e in range(NUM_EPOCHS):
    for batch, (source_seq, target_seq_in, target_seq_out) in enumerate(
        dataset.take(-1)
    ):
        loss = train_step(source_seq, target_seq_in, target_seq_out)
        if batch % 100 == 0:
            print(
                "Epoch {} Batch {} Loss {:.4f} Elapsed time {:.2f}s".format(
