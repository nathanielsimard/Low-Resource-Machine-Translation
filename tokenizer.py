...
def main():
    ...
    parser.add_argument(
        "--keep-empty-lines", help="will keep empty lines", action="store_true"
    )
    parser.add_argument(
        "--newline-to-space", help="converts newlines to spaces", action="store_true"
    )
    parser.add_argument(
        "--skip-lines-with-pattern",
        nargs="*",
        default=[],
    ...
    logger.info(
        "will skip lines with the following regex: {}".format(
    ...
    if args.lang == "en":
        try:
            ...
            raise ValueError(
                "please run the following to download the model: "
    ...
    for current_file in args.input:
        ...
        logger.info(
            "done ({} / {}) - skipped {} lines (over {} - i.e., {:3.2f}%) because empty,"
...
def tokenize(
    current_file, output, tokenizer, keep_case, keep_empty_lines, newline_to_space, regs
):
