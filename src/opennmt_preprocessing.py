...
def build_vocabulary(
    input_file: str, output_file: str, size: Union[int, None]
):  # Test Covered
...
def prepare_bpe_models(
    ...
    """
    src_prefix, tgt_prefix = get_bpe_prefix(combined=combined, model_dir=model_dir)
    if not combined:
...
def shuffle_file(filename: str, inplace=True, seed=1234):  # Test covered
...
) -> Tuple[str, str]:  # Test Covered indirectly
...
def get_bpe_vocab_files(
    combined=False, prefix="default", model_dir="checkpoint"
) -> Tuple[str, str]:  # Test Covered indirectly
    """Get the path to the BPE vocabulary files (.vocab).

    Vocabulary files are not to be confused with the BPE
    vocabulary files. Those files are used py the OpenNMT
    inputer module, while the BPE vocabulary files are used
    by SentencePiece.
    ...
    Keyword Arguments:
        combined {bool} : use combined vocabulary (default: {False})
        prefix {str} : arbitrary additional prefix. (default: {"default"})
        model_dir {str}: folder where to save the model. Ideally checkpoint folder.

    Returns:
        (str,str): Path to source and target BPE vocabulary files.
    """
    src_bpe_prefix, tgt_bpe_prefix = get_bpe_prefix(
        prefix=prefix, combined=combined, model_dir=model_dir
    )
    return (src_bpe_prefix + ".vocab", tgt_bpe_prefix + ".vocab")


def get_bpe_model_files(
    combined=False, prefix="default", model_dir="checkpoint"
) -> Tuple[str, str]:  # Test Covered indirectly
...
def prepare_bpe_files(
    ...
    """
    src_model, tgt_model = get_bpe_model_files(
        combined=combined, prefix=prefix, model_dir=model_dir
    )
    ...
    if target_file is not None:
        ...
        bpe_target_file = None
    return bpe_source_file, bpe_target_file


def decode_bpe_file(
    filename: str, target=True, combined=False, prefix="default", model_dir="checkpoint"
) -> str:  # Test covered
...
def main():
    """Build ONMT Vocabulary from command line."""
