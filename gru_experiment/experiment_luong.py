...
def compute_bleu(pred_file_path, target_file_path):
    ...
    scores = [0, 0]
    if 'no sentence' not in out.stdout:
...
class LuongAttention(tf.keras.layers.Layer):
    def __init__(self, units):
        ...
        self.attention = tf.keras.layers.Attention(units)
    ...
    def call(self, query, values):
        ...
        attention_weights = self.attention([query_with_time_axis, values])
        context_vector = attention_weights * values
        context_vector = tf.reduce_sum(context_vector, axis=1)
        return context_vector, attention_weights
...
#import tensorflow_addons.seq2seq.LuongAttention as LuongAttention
...
def main():
    train_lines = read_file('gru_experiment/test.train')
