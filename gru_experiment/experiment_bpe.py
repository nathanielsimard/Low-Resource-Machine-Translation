...
def prepare_tokenizer(lang, vocab_size=2000):
    tokenizer = ByteLevelBPETokenizer()
    with open('/tmp/lang.txt', 'a') as the_file:
        for sentence in lang:
            the_file.write(sentence + "\n")
    tokenizer.train(["/tmp/lang.txt"], vocab_size)
    tokenizer.add_special_tokens(["<start>", "<end>"])
    return tokenizer

def tokenize(lang):

    lang_tokenizer = prepare_tokenizer(lang)

    tensor = [lang_tokenizer.encode(sentence).ids for sentence in lang]
...
def train_step(inp, targ, enc_hidden, encoder, decoder, targ_lang, BATCH_SIZE, loss_object, optimizer):
    ...
    with tf.GradientTape() as tape:
        ...
        dec_input = tf.expand_dims([targ_lang.token_to_id('<start>')] * BATCH_SIZE, 1)
...
def compute_pred_tar(pred, tar, targ_lang_tokenizer):
    ...
    for fa in tf.transpose(tar):
        ...
        eos = numpy.where(x == targ_lang_tokenizer.token_to_id('<end>'))[0][0]
        target = targ_lang_tokenizer.decode(x[:eos])
    ...
    for tr in tf.transpose(pred, [1, 0, 2]):
        prediction = []
        x = np.argmax(tr,axis=1)
        if len(numpy.where(x == targ_lang_tokenizer.token_to_id('<end>'))[0])==0:
            eos = len(x)
        else:
            eos = numpy.where(x == targ_lang_tokenizer.token_to_id('<end>'))[0][0]
        prediction = targ_lang_tokenizer.decode(x[:eos])
...
def main():
    train_lines = read_file('gru_experiment/test.train4000')
    ...
    embedding_dim = 256
    units = 1024
    vocab_inp_size = inp_lang_tokenizer.get_vocab_size()
    vocab_tar_size = targ_lang_tokenizer.get_vocab_size()
